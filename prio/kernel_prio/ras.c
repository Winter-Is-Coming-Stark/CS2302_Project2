...
void init_ras_rq(struct ras_rq *ras_rq, struct rq *rq){
    ...
    __set_bit(MAX_RAS_PRIO, array->bitmap);
...
static inline int ras_se_prio(struct sched_ras_entity *ras_se){
    return ras_task_of(ras_se)->ras_prio;
}
...
static unsigned int get_timeslice(struct rq *rq, struct task_struct *task){
    ...
    TODO!
    */
    if(task->ras_prio != MAX_RAS_PRIO -1) return RAS_TIMESLICE_MAX;
    unsigned int prob = 0;
	unsigned int time_slice = 0;
    prob = get_proba(rq, task);
	time_slice = RAS_TIMESLICE_MAX - prob;
...
static void requeue_ras_entity(struct ras_rq *ras_rq, struct sched_ras_entity *ras_se, int head){
    if(on_ras_rq(ras_se)){
        ...
        queue = array->queue + ras_se_prio(ras_se);
